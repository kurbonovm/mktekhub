...
/**
 * Comprehensive unit tests for DataInitializer.
 * Tests initialization of default roles, warehouses, and sample inventory items.
 */
...
class DataInitializerTest {
    ...
    void run_ShouldCreateAdminRole() throws Exception {
        ...
        Role adminRole = capturedRoles.stream()
    ...
    void run_ShouldCreateManagerRole() throws Exception {
        ...
        List<Role> capturedRoles = roleCaptor.getAllValues();
        Role managerRole = capturedRoles.stream()
            .filter(r -> "MANAGER".equals(r.getName()))
            .findFirst()
            .orElse(null);
    ...
    void run_ShouldCreateViewerRole() throws Exception {
        ...
        List<Role> capturedRoles = roleCaptor.getAllValues();
        Role viewerRole = capturedRoles.stream()
            .filter(r -> "VIEWER".equals(r.getName()))
            .findFirst()
            .orElse(null);
    ...
    void run_ShouldSeedWarehousesWhenCountIsZero() throws Exception {
        ...
        when(warehouseRepository.findByName("West Coast Distribution Center")).thenReturn(Optional.of(westCoast));
    ...
    void run_ShouldCreateMainWarehouseWithCorrectProperties() throws Exception {
        ...
        when(warehouseRepository.findByName("West Coast Distribution Center")).thenReturn(Optional.of(new Warehouse()));
        ...
        Warehouse mainWarehouse = warehouseCaptor.getAllValues().stream()
    ...
    void run_ShouldCreateWestCoastDistributionCenter() throws Exception {
        ...
        Warehouse warehouse = warehouseCaptor.getAllValues().stream()
    ...
    void run_ShouldCreateInventoryItemsWithVariousCategories() throws Exception {
        ...
        assertTrue(items.stream().anyMatch(item -> item.getSku() != null && item.getSku().startsWith("ELEC-")));
    }
