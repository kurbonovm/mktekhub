...
class WarehouseControllerTest {
  ...
  void getAllWarehouses() throws Exception {
    when(warehouseService.getAllWarehouses())
        .thenReturn(Collections.singletonList(warehouseResponse));
  ...
  void getActiveWarehouses() throws Exception {
    when(warehouseService.getActiveWarehouses())
        .thenReturn(Collections.singletonList(warehouseResponse));

    mockMvc.perform(get("/api/warehouses/active")).andExpect(status().isOk());
  ...
  void getWarehousesWithAlerts() throws Exception {
    ...
    mockMvc.perform(get("/api/warehouses/alerts")).andExpect(status().isOk());
  ...
  void deleteWarehouse() throws Exception {
    ...
    mockMvc
        .perform(delete("/api/warehouses/1"))
        .andExpect(status().isOk())
  ...
  void hardDeleteWarehouse() throws Exception {
    ...
    mockMvc
        .perform(delete("/api/warehouses/1/permanent"))
        .andExpect(status().isOk())
