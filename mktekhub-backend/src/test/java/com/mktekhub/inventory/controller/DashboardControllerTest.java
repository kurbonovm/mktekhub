...
class DashboardControllerTest {
  ...
  void getWarehouseSummary() throws Exception {
    DashboardSummary.WarehouseSummary summary =
        new DashboardSummary.WarehouseSummary(
    ...
    mockMvc
        .perform(get("/api/dashboard/warehouse-summary"))
  ...
  void getInventorySummary() throws Exception {
    DashboardSummary.InventorySummary summary =
        new DashboardSummary.InventorySummary(100, 5000, BigDecimal.valueOf(250000), 50, 10);
    when(dashboardService.getInventorySummary()).thenReturn(summary);

    mockMvc
        .perform(get("/api/dashboard/inventory-summary"))
        .andExpect(status().isOk())
  ...
  void getAlertsSummary() throws Exception {
    DashboardSummary.AlertsSummary summary = new DashboardSummary.AlertsSummary(10, 5, 8, 2);
    when(dashboardService.getAlertsSummary()).thenReturn(summary);

    mockMvc
        .perform(get("/api/dashboard/alerts-summary"))
        .andExpect(status().isOk())
