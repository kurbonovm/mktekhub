...
class InventoryItemControllerTest {
    ...
    void getItemBySku() throws Exception {
        ...
        mockMvc.perform(get("/api/inventory/sku/SKU-001"))
            .andExpect(status().isOk())
    ...
    void getItemsByWarehouse() throws Exception {
        when(inventoryItemService.getItemsByWarehouse(1L)).thenReturn(Collections.singletonList(itemResponse));
    ...
    void getItemsByCategory() throws Exception {
        when(inventoryItemService.getItemsByCategory("Electronics")).thenReturn(Collections.singletonList(itemResponse));
    ...
    void getLowStockItems() throws Exception {
        when(inventoryItemService.getLowStockItems()).thenReturn(Collections.singletonList(itemResponse));
    ...
    void getItemsExpiringSoon_CustomDays() throws Exception {
        ...
        mockMvc.perform(get("/api/inventory/alerts/expiring?days=60"))
            .andExpect(status().isOk());
    ...
    void updateItem() throws Exception {
        when(inventoryItemService.updateItem(eq(1L), any(InventoryItemRequest.class))).thenReturn(itemResponse);
    ...
    void getItemBySku_NotFound() throws Exception {
        ...
        mockMvc.perform(get("/api/inventory/sku/INVALID"))
            .andExpect(status().isNotFound());
    ...
    void getItemsByCategory_EmptyList() throws Exception {
        ...
        mockMvc.perform(get("/api/inventory/category/NonExistent"))
            .andExpect(status().isOk())
            .andExpect(jsonPath("$").isEmpty());
    ...
    void deleteItem_NotFound() throws Exception {
        doThrow(new ResourceNotFoundException("InventoryItem", "id", 999L))
            .when(inventoryItemService).deleteItem(999L);
    ...
    void adjustQuantity_NotFound() throws Exception {
        ...
        mockMvc.perform(patch("/api/inventory/999/adjust")
                .contentType(MediaType.APPLICATION_JSON)
                .content(objectMapper.writeValueAsString(request)))
            .andExpect(status().isNotFound());
