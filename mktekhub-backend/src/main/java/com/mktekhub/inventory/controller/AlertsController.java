...
public class AlertsController {
  ...
  /** Get all alerts in one response Accessible by ADMIN and MANAGER */
  @GetMapping("/all")
  @PreAuthorize("hasAnyRole('ADMIN', 'MANAGER')")
  @Operation(
      summary = "Get all alerts",
      description =
          "Get all types of alerts including low stock, expired items, expiring soon items, and capacity alerts")
  public ResponseEntity<CombinedAlertsResponse> getAllAlerts(
    ...
    List<InventoryItemResponse> expiringSoonItems =
        inventoryItemService.getItemsExpiringSoon(expiringDays);
    List<WarehouseResponse> capacityAlerts = warehouseService.getWarehousesWithAlerts();

    CombinedAlertsResponse response =
        new CombinedAlertsResponse(lowStockItems, expiredItems, expiringSoonItems, capacityAlerts);
