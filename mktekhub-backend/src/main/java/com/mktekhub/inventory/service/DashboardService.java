...
public class DashboardService {
    ...
    public DashboardSummary.WarehouseSummary getWarehouseSummary() {
        ...
        if (!allWarehouses.isEmpty()) {
            BigDecimal totalUtilization = allWarehouses.stream()
                .map(Warehouse::getUtilizationPercentage)
                .reduce(BigDecimal.ZERO, BigDecimal::add);
            averageUtilization = totalUtilization.divide(
                BigDecimal.valueOf(allWarehouses.size()),
                2,
    ...
    public DashboardSummary.InventorySummary getInventorySummary() {
        ...
        long uniqueSkus = allItems.stream()
    ...
    public DashboardSummary.AlertsSummary getAlertsSummary() {
        ...
        long expiringSoonItems = inventoryItemRepository.findByExpirationDateBetween(
            LocalDate.now(),
            thirtyDaysFromNow
        ).size();
