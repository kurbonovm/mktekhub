...
public class InventoryItemService {
  ...
  public InventoryItemResponse createItem(InventoryItemRequest request) {
    ...
    Warehouse warehouse =
        warehouseRepository
            ...
            .orElseThrow(
                () -> new ResourceNotFoundException("Warehouse", "id", request.getWarehouseId()));

    // Calculate total volume for this item
    BigDecimal volumePerUnit =
        request.getVolumePerUnit() != null ? request.getVolumePerUnit() : BigDecimal.ZERO;
    ...
    if (warehouse.wouldExceedCapacity(totalVolume)) {
      throw new WarehouseCapacityExceededException(
          warehouse.getName(), warehouse.getAvailableCapacity(), totalVolume);
    }
  ...
  public InventoryItemResponse updateItem(Long id, InventoryItemRequest request) {
    ...
    BigDecimal newVolumePerUnit =
        request.getVolumePerUnit() != null ? request.getVolumePerUnit() : BigDecimal.ZERO;
    BigDecimal newTotalVolume =
        newVolumePerUnit.multiply(BigDecimal.valueOf(request.getQuantity()));
    ...
    if (!item.getWarehouse().getId().equals(request.getWarehouseId())) {
      ...
      if (newWarehouse.wouldExceedCapacity(newTotalVolume)) {
        throw new WarehouseCapacityExceededException(
            newWarehouse.getName(), newWarehouse.getAvailableCapacity(), newTotalVolume);
      }
      ...
      if (volumeDifference.compareTo(BigDecimal.ZERO) > 0
          && oldWarehouse.wouldExceedCapacity(volumeDifference)) {
        throw new WarehouseCapacityExceededException(
            oldWarehouse.getName(), oldWarehouse.getAvailableCapacity(), volumeDifference);
      }
  ...
  public InventoryItemResponse adjustQuantity(Long id, Integer quantityChange) {
    ...
    BigDecimal volumePerUnit =
        item.getVolumePerUnit() != null ? item.getVolumePerUnit() : BigDecimal.ZERO;
    ...
    if (volumeChange.compareTo(BigDecimal.ZERO) > 0
        && item.getWarehouse().wouldExceedCapacity(volumeChange)) {
      throw new WarehouseCapacityExceededException(
          item.getWarehouse().getName(), item.getWarehouse().getAvailableCapacity(), volumeChange);
    }
    ...
    User currentUser = getCurrentUser();
    activityLoggerService.logAdjustment(
        updated, quantityChange, oldQuantity, newQuantity, currentUser);
