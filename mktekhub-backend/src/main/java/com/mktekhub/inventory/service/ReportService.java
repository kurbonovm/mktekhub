...
public class ReportService {
  ...
  public byte[] exportInventoryToCSV() {
    ...
    // CSV Header
    writer.println(
        "SKU,Name,Category,Brand,Quantity,Unit Price,Warehouse,Location,Reorder Level,Barcode,Expiration Date,Warranty End Date");

    // CSV Data
    for (InventoryItem item : items) {
      writer.printf(
          "%s,%s,%s,%s,%d,%.2f,%s,%s,%d,%s,%s,%s%n",
          ...
          item.getWarrantyEndDate() != null
              ? item.getWarrantyEndDate().format(DATE_FORMATTER)
              : "");
    }
  ...
  public byte[] exportWarehousesToCSV() {
    ...
    // CSV Header
    writer.println(
        "Name,Location,Max Capacity,Current Capacity,Utilization %,Available Capacity,Alert Threshold %,Status,Created At");

    // CSV Data
    for (Warehouse warehouse : warehouses) {
      writer.printf(
          "%s,%s,%.2f,%.2f,%.2f,%.2f,%.2f,%s,%s%n",
  ...
  public byte[] exportStockActivitiesToCSV() {
    ...
    // CSV Header
    writer.println(
        "Timestamp,Item SKU,Item Name,Activity Type,Quantity Change,Previous Quantity,New Quantity,Source Warehouse,Destination Warehouse,Performed By,Notes");

    // CSV Data
    for (StockActivity activity : activities) {
      writer.printf(
          "%s,%s,%s,%s,%d,%d,%d,%s,%s,%s,%s%n",
          ...
          activity.getSourceWarehouse() != null
              ? escapeCsv(activity.getSourceWarehouse().getName())
              : "",
          activity.getDestinationWarehouse() != null
              ? escapeCsv(activity.getDestinationWarehouse().getName())
              : "",
  ...
  public byte[] generateLowStockReport() {
    ...
    List<InventoryItem> lowStockItems =
        items.stream()
            .filter(
                item ->
                    item.getReorderLevel() != null && item.getQuantity() <= item.getReorderLevel())
  ...
  public byte[] generateWarehouseUtilizationReport() {
    ...
    // CSV Header
    writer.println(
        "Warehouse,Location,Max Capacity,Current Capacity,Available Capacity,Utilization %,Status,Alert Status");
  ...
  public byte[] generateStockMovementReport() {
    ...
    // CSV Header
    writer.println(
        "Date,Time,Activity Type,Item SKU,Item Name,Quantity,Source Warehouse,Destination Warehouse,Performed By");
  ...
  public byte[] generateInventorySummaryByCategory() {
    ...
    for (Map.Entry<String, List<InventoryItem>> entry : itemsByCategory.entrySet()) {
      ...
      writer.printf("%s,%d,%d,%.2f%n", escapeCsv(category), totalItems, totalQuantity, totalValue);
    }
  ...
  public byte[] generateFilteredStockActivityReport(
    ...
    // Apply filters
    List<StockActivity> filteredActivities =
        activities.stream()
            .filter(
                activity -> {
                  // Date range filter
                  if (startDate.isPresent()
                      && activity.getTimestamp().toLocalDate().isBefore(startDate.get())) {
                    return false;
                  }
                  if (endDate.isPresent()
                      && activity.getTimestamp().toLocalDate().isAfter(endDate.get())) {
                    return false;
                  }
                  ...
                  if (warehouseId.isPresent()) {
                    ...
                    if (activity.getSourceWarehouse() != null
                        && activity.getSourceWarehouse().getId().equals(warehouseId.get())) {
                      matchesWarehouse = true;
                    }
                    if (activity.getDestinationWarehouse() != null
                        && activity.getDestinationWarehouse().getId().equals(warehouseId.get())) {
                      matchesWarehouse = true;
                    }
                    if (!matchesWarehouse) {
                      return false;
                  ...
                  if (activityType.isPresent()
                      && !activity.getActivityType().name().equals(activityType.get())) {
                    return false;
                  }
    ...
    // CSV Header
    writer.println(
        "Date,Time,Activity Type,Item SKU,Item Name,Category,Brand,Quantity,Source Warehouse,Destination Warehouse,Performed By");
  ...
  public byte[] generateFilteredInventoryValuationReport(
      Optional<String> category, Optional<String> brand, Optional<Long> warehouseId) {
        ...
        items.stream()
            .filter(
                item -> {
                  ...
                  // Warehouse filter
                  if (warehouseId.isPresent()
                      && !item.getWarehouse().getId().equals(warehouseId.get())) {
                    return false;
                  }
