...
public class StockActivityService {
  ...
  /** Get activities with multiple filters All parameters are optional (nullable) */
  public List<StockActivityResponse> getActivitiesWithFilters(
    ...
    if (startDate != null && endDate != null) {
      activities =
          activities.stream()
              .filter(
                  a -> !a.getTimestamp().isBefore(startDate) && !a.getTimestamp().isAfter(endDate))
              .collect(Collectors.toList());
    ...
    return activities.stream().map(StockActivityResponse::fromEntity).collect(Collectors.toList());
  }
}
